package com.maitsev.authservice.jwt;
...
import com.maitsev.authservice.config.MyUserDetailsService;
...
public class JwtService {
    ...
    private String createToken(Map<String, Object> claims, String userName, UUID userId) {
        ...
        // After extracting the authorities/roles and removing the "[" "]", we are adding the authorities/roles to the claim
        ...
        return Jwts.builder() // Adds all given name/value pairs to the JSON Claims in the payload.
                ...
                .setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * 60)) // when setting an expiration "Date", we use milliseconds, the easiest way to set it is using the following format 1000(1000 milliseconds = second) * 60 (1 second * 60 = a minute) * 60 (1 minute * 60 = 1 hour) -> 1000 * 60 * 60 = 1 hour
                ...
                // For the key, we are using a function to create it, and for the SignatureAlgorithm we are using the "HS256" hashing algorithm.
    ...
    // A function to extract the user id of the token
